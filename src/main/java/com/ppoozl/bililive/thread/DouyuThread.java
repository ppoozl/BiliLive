...
public class DouyuThread extends BaseBiliLiveThread {
    ...
    public void doRun() {
        // 获取真实房间 ID
        String roomID = BiliLiveConfig.room.get().toString();
    ...
    private boolean isReachable() {
        try {
            return InetAddress.getByName(LIVE_URL).isReachable(BiliLiveConfig.timeout.get());
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private int getHotValue() {
        ...
        try {
            // B 站提供的获取直播 id 的 api
            URL url = new URL("http://open.douyucdn.cn/api/RoomApi/room/" + BiliLiveConfig.room.get());
            ...
            if (matcher.find()) {
                ...
                // 日志记录
                BiliLive.logger.fatal("Cannot get live hot value.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
